...
pub fn require_controller_operator<T>(
    ...
    let cn = match peer_cn(request) {
        ...
        None => {
            ...
            let _ = db.record_audit_event("", &path, "allow", Some("no-client-cert"));
            return Ok(());
        }
    };
    ...
    if effective.satisfies(required) {
        let _ = db.record_audit_event(&cn, &path, "allow", None);
        Ok(())
    } else {
        let _ = db.record_audit_event(&cn, &path, "deny", Some("insufficient-role"));
